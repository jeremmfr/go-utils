...
func TestAbsolutInt(t *testing.T) {
	...
	if v := basicalter.AbsoluteInt(i); v < 0 {
		t.Errorf("AbsoluteInt(%d) returned negative integer: %d", i, v)
	}
	if v := basicalter.AbsoluteInt(basicalter.AbsoluteInt(i)); v < 0 {
		t.Errorf("AbsoluteInt(AbsoluteInt(%d)) returned negative integer: %d", i, v)
	}

	i = minInt + 1
	if v := basicalter.AbsoluteInt(i); v != maxInt {
		t.Errorf("AbsoluteInt(%d) didn't return expected integer %d, found %d", i, maxInt, v)
	}
}
...
func TestAbsolutIntPanic(t *testing.T) {
	...
	t.Errorf("AbsoluteInt didn't panic with the minimum value of integers")
}
